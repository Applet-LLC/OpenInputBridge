...
無改変の`samples/x2y/x2y.cpp`（`X2YSample.vcxproj`）。`caps2esc`のように状態を持たず、
Xキーが押されたら単純にYキーに書き換えるだけの、最小構成のキーボードリマップサンプル。
これで`third_party/interception/samples/`配下の全サンプルをカバーした。
...
実行した状態でXキーを押すとYが入力されることを確認する。Escで終了。

## 複数ツール同時アタッチの確認（実機、2026-07-30）

`x2y.exe`の動作確認中に`cadstop.exe`を起動したままにしていたところ、両方問題なく機能した
（Xキーのリマップも、Ctrl+Alt+Delの捕捉も、どちらも動作）。`precedence_probe`による意図的な
テスト（`tests/precedence_blackbox/README.md`参照）とは別に、**無関係な別々のツールを
同時にアタッチしても互いに干渉せず動作する**ことが実際の利用シナリオに近い形で確認できた
（どちらも明示的なprecedence指定なし＝デフォルト0のフックチェーンとして自然に共存している）。
