...
### 2026-08-02: 未検証の前提を追加（`IOCTL_GET_DRIVER_IDENTITY`）

新規クライアント向けの独自拡張IOCTL `IOCTL_GET_DRIVER_IDENTITY`（`docs/PROTOCOL.md`参照）は、
「本物のInterceptionドライバは未知のIOCTLコードを安全に（クラッシュ等せず）拒否するはず」という
前提の上に設計している。この前提はWDFの未処理IOCTLに対する一般的なデフォルト動作・通常の
防御的実装作法から期待される挙動であり、**実物のInterceptionドライバでこの反応を確認したもの
ではない**。上記のクリーンルーム方針（外部から観測可能なI/O動作のみのブラックボックス観察は
許可、バイナリの逆アセンブル等は禁止）に照らせば、この確認自体は許可された範囲の作業
（未知のIOCTLコードを送り、返ってくるステータスを観察するだけ）のはずだが、現時点では
未実施のため、`docs/PROTOCOL.md`にブラックボックス検証待ちの項目として明記した
（precedenceのビット照合規則と同様の扱い）。実施時はこのセクションに実施日・観察内容・
結論を追記する。
