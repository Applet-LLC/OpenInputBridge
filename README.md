...
また、スマート アプリ コントロール（Smart App Control、英語環境ではこの表記）が有効な場合、
テスト署名を有効にしていても、WHQL未認定のドライバは読み込みがブロックされます
（デバイス マネージャーでは「コード39」、イベントビューアーの
「アプリケーションとサービスログ」→「Microsoft」→「Windows」→「CodeIntegrity」→
「Operational」ではイベントID 3077/3004として記録されます）。設定の「プライバシーと
セキュリティ」→「Windows セキュリティ」→「アプリとブラウザー コントロール」から
オフにしてください。**一度オフにすると、Windowsを再インストール/リセットするまで
再度オンには戻せません。** 特に近年のARM64機を含む新規Windows 11インストールでは
既定でオンになっていることが多い点に注意してください。

WHQL署名済みドライバではこれらの手順はいずれも不要です。
...
なります。**OneDriveなどクラウド同期されるフォルダ（同期対象になっている「デスクトップ」
フォルダ等を含む）の配下には展開しないでください**。監査ログのSACLを毎回の起動時に
再適用する`OpenInputBridgeAuditLogReapply`タスクはSYSTEM権限・起動時（ユーザーのサインイン
セッションが確立する前）に動作しますが、実機検証で、OneDrive同期フォルダ配下にインストール
した場合この再適用が実行されず（Files On-Demandによるプレースホルダー化等が影響している
とみられます）、結果として監査ログ・トースト通知機能が全く動作しないことを確認しています。
`C:\OpenInputBridge`のような、クラウド同期されないローカルパスへの展開を推奨します。
